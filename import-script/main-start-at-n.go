...
const (
	...
	CLASS_NAME    = "Vitrivr"
	START_VECTOR  = 25000000 // Start at vector 25 million
)
...
func readBvecsFile(ctx context.Context, filename string, vectorChan chan<- VectorObject) error {
	...
	// Skip to the starting vector position by seeking
	if START_VECTOR > 0 {
		log.Printf("Seeking to vector %d...", START_VECTOR)

		// Read the first vector's dimension to determine vector size
		var dim uint32
		...
		// Calculate bytes per vector: 4 bytes for dimension + dim bytes for data
		bytesPerVector := int64(4 + dim)

		// Calculate the target byte position
		targetOffset := int64(START_VECTOR) * bytesPerVector

		// Seek to the target position
		_, err = file.Seek(targetOffset, io.SeekStart)
		if err != nil {
			return fmt.Errorf("failed to seek to vector %d: %v", START_VECTOR, err)
		}

		// Reset the reader since we moved the file position
		reader.Reset(file)

		log.Printf("Seeked to byte offset %d (vector %d)", targetOffset, START_VECTOR)
		vectorID = START_VECTOR
	}
	...
	log.Printf("Finished reading %d vectors from file", vectorID-START_VECTOR)
	return nil
}
